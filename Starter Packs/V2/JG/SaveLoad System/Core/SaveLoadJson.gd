...
func _process(delta):
	if Input.is_action_just_released("save"):
		capture_state("GameSave","Test")
	elif Input.is_action_just_released("load"):
		restore_state("Test")
	pass

func capture_state(save_type,to_file):
	var savables = get_tree().get_nodes_in_group(save_type)
	var state = {save_type_key : save_type}
...
func restore_state(from_file):
	...
	var save_type = saved_data[save_type_key]
	
	$DataDistributor.send_data(saved_data,save_type)
	pass
